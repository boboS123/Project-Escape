...
bool engine::CreateSwapChainDx12(HWND windowHwnd)
{
	...
	swapChainDesc.Windowed = 0; // set to true, then if in fullscreen must call SetFullScreenState with true for full screen to get uncapped fps
...
//		OutputDebugStringA((char*)errorBuff->GetBufferPointer());
	...
	loadMesh->LoadChunk(sizeof(int), (numMeshesOnFile* (sizeof(int) + 6 * (sizeof(float)) + sizeof(int) + sizeof(DWORD) * 3)));
...
void engine::Render()
{
	...
	//Object
	commandList->IASetVertexBuffers(0, 1, mgr.Dx12ActiveObjects[0].vertexBufferView); // set the vertex buffer (using the vertex buffer view)
