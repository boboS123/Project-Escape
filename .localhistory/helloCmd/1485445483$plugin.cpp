...
bool bobCATexporter::ExtractMeshDataToBuffer(MFnMesh &mesh, MESHLAYOUT layout, DWORD &size,
	...
	if ((bitFieldVertexFormat&vertexFormat_tex2) == vertexFormat_nrm3) {
		float_elements += 3;
	}
...
int bobCATexporter::ExportAllMeshes(bool compressed, XVector<MeshInfo> &sceneInfo) {
	...
	while (!it.isDone())
	{
		...
		ExtractMeshDataToBuffer(fn, POS4COLOR4, size, points, normals, tangents, u_coords, v_coords, &data, numVerts, POS4_TEX2_NORM3, vertexFormat);
